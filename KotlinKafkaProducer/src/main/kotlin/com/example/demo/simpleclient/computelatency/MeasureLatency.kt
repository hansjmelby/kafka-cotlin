package com.example.demo.simpleclient.computelatency

import com.ippontech.kafkatutorials.simpleclient.Person
import com.example.demo.simpleclient.withcustomserde.PersonDeserializer
...
class LatencyAnalyzer(val brokers: String, val inputTopic: String, val outputTopic: String) {
    ...
    private val logger = Logger.getLogger("LatencyAnalyzer")
    ...
    private fun readFromInputTopic() {
        ...
        logger.log(Level.INFO,"Received ${records.count()} records from the input topic")
    ...
    private fun readFromOutputTopic() {
        ...
        logger.log(Level.INFO,"Received ${records.count()} records from the output topic")
        records.iterator().forEach {
            val key = it.key()
